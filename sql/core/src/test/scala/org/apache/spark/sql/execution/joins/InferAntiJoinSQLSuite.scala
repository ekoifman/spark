...
class InferAntiJoinSQLSuite extends QueryTest
  ...
  test("infer antijoin on clause on one colum is null another but the same through aliasing") {
    val left = spark.range(2)
    val right = spark.range(2)
    for (inferAntiJoin <- Seq(true, false)) {
      withSQLConf(SQLConf.INFER_ANTI_JOIN.key -> inferAntiJoin.toString) {
  ...
  test("leftouterjoin edftrading") {
    val leftSideRows = 11 // 10000
    val rightSideRows = 10 // 1000000
    val left = spark.range(leftSideRows)
    val right = spark.range(rightSideRows)
    for (inferAntiJoin <- Seq(false, true)) {
      for (useHashJoin <- Seq(false, true)) {
        withSQLConf(
          SQLConf.WHOLESTAGE_CODEGEN_ENABLED.key -> "false",
          SQLConf.ADAPTIVE_EXECUTION_ENABLED.key -> "true",
          // to test with Shuffle joins
          SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "-1"
          // this is to make the rule visible in Compiler Stats
//          , SQLConf.UI_RULE_SHOW.key -> "145"
          , SQLConf.SHUFFLE_PARTITIONS.key -> "8"
          , SQLConf.INFER_ANTI_JOIN.key -> inferAntiJoin.toString
        ) {
          val df1 = left.selectExpr("cast(id%2 as string) as c1")
            .selectExpr("c1", "c1 as c2", "c1 as c3", "c1 as c4", "c1 as c5", "c1 as c6",
              "c1 as c7", "c1 as c8")

          val df = {
            val df2 = right.selectExpr("cast(id%2 as string) r1")
            df1.join(if (useHashJoin) df2.hint("shuffle_hash") else df2,
                $"c1" === $"r1", "leftOuter").filter("r1 is null")
              .select(df1.columns.head, df1.columns.toIndexedSeq: _*)
          }
          checkIt(df, inferAntiJoin, Array[Row](), !inferAntiJoin)
        }
      }
